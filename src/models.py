...
class EigmodePreprocessing(tf.keras.layers.Layer):
    ...
    def call(self, csm):
        ...
        if self.concat_real_imag:
            ...
            eigmode = tf.reshape(eigmode, [-1, input_shape[1],input_shape[2]*input_shape[3]])
        return eigmode
...
class MLPViT(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_size, dropout_rate, num_layers=2):
        ...
        self.dropout_layers = [tf.keras.layers.Dropout(dropout_rate)
            for _ in range(num_layers)]
...
class ViTEncoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, name, num_heads, key_dim, dropout, version=2, hidden_dim=None):
        ...
        self.mhsa = MultiHeadSelfAttention(num_heads, key_dim, dropout, version=version, use_causal_mask=False)
        self.mlp = TransformerMLP(key_dim=key_dim, hidden_dim=hidden_dim, dropout=dropout, version=version)
...
class TransformerEncoder(tf.keras.Model):
    ...
    def __init__(self, version, num_layers, key_dim, num_heads,
                hidden_dim=None, dropout_rate=0.1):
        super().__init__()
...
class EigmodeTransformer(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.encoder = TransformerEncoder(
            version=2, num_layers=12, key_dim=self.nchannels*2, num_heads=12,
            dropout_rate=dropout_rate) if encoder is None else encoder
        # positional embedding
        if self.pos_enc:
            self.positional_encoding = tf.keras.layers.Embedding(
                input_dim=self.max_seq_len,
                output_dim=self.encoder.key_dim,
                weights=[get_pos_encoding_matrix(self.max_seq_len,self.encoder.key_dim)],
                trainable = False,
        ...
        self.strength_head = tf.keras.layers.Dense(
            units=nsources, activation='linear')
        self.loc_head = [tf.keras.layers.Dense(
            units=nsources, activation='linear') for _ in range(dim)]
        if noise_power:
            self.noise_head = tf.keras.layers.Dense(
                units=1, activation='linear')
        self.dense_head = MLPViT(
            dropout_rate=dropout_rate, hidden_size=self.encoder.key_dim*4, num_layers=hidden_layers)
    ...
    def call(self, inputs):
        ...
        if self.pos_enc:
            encodings = self.positional_encoding(tf.range(start=0, limit=self.max_seq_len, delta=1))
            inputs += encodings
        ...
        x = self.dense_head(x)
        loc = tf.concat([self.loc_head[i](x)[:, tf.newaxis, :]
                        for i in range(self.dim)], axis=1)
        if self.noise_power:
            p2 = tf.nn.softmax(tf.concat([self.strength_head(x), self.noise_head(x)], axis=-1))
        else:
    ...
    def get_assignment(self, loc_true, loc_pred):
        def loop_body(x):
            ...
            J = tf.shape(Rho)[1]
            tau = tf.cast(tf.math.floordiv(I,J),tf.int16) # blanced or near-balanced assignment
            T = tf_constrained_kmeans(Rho,tau)
            # Padding with zeros
            T = tf.pad(T, [[0, 0], [0, I-J]], 'CONSTANT', constant_values=0.)
            Rho = tf.pad(Rho, [[0, 0], [0, I-J]], 'CONSTANT', constant_values=0.)
            return T, Rho
        T, Rho = tf.map_fn(
            loop_body,elems=(tf.range(self.batchsize), loc_true, loc_pred),
            fn_output_signature=(
    ...
    def matching_loss(self, T, Rho, alpha_true, alpha_pred):
        ...
        loss = tf.reduce_sum((alpha_true[:,:self.nsources]-estimate_sum)**2,axis=1)
        # noise loss
        loss += tf.reduce_sum((alpha_true[:,self.nsources:]-alpha_pred[:,self.nsources:])**2,axis=1)
        loss *= self.lam
        # loc loss
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            (alpha_pred, loc_pred, noise_pred) = self(csm, training=True)  # Forward pass
            T, Rho = self.get_assignment(loc_true, loc_pred) # abs in case of rounding errors
            if self.noise_power:
        ...
        if hasattr(self, 'compiled_metrics') and self.compiled_metrics is not None:
            self.compiled_metrics.update_state((alpha_true, loc_true), (alpha_pred, loc_pred))
