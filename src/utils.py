...
class Scheduler(tf.optimizers.schedules.LearningRateSchedule):
    def __init__(self, nominal_value, n_epochs, warmup_epochs, decay_epochs,
                 steps_per_epoch):
        super().__init__()
    ...
    def initial_value(self):
        return tf.cast(self.nominal_value/self.warmup_steps*0.5,
                       tf.float64)

    def linear_ramp(self, step):
        step = tf.cast(step, tf.float64)
        return tf.cast(tf.minimum(self.nominal_value/self.warmup_steps*step,
                                  self.nominal_value), tf.float64)

    def exponential_decay(self, step):
        factor = tf.cast(step-self.start_decay_step, tf.float64)
        return tf.exp(tf.cast(-0.05 / self.steps_per_epoch,
                              tf.float64)*factor)*self.nominal_value

    def __call__(self, step):
        step = tf.cast(step, tf.int64)
        val = tf.cond(step == tf.cast(0, tf.int64),
                      lambda: self.initial_value(),
                      lambda: self.linear_ramp(step))
        val = tf.cond(step < self.start_decay_step,
                      lambda: val,
...
class LRScheduler(Scheduler):
    def __init__(self, learning_rate, n_epochs, warmup_epochs, decay_epochs,
                 steps_per_epoch: int = 2000):
        super().__init__(nominal_value=learning_rate, n_epochs=n_epochs,
...
class WDScheduler(Scheduler):
    def __init__(self, weight_decay, n_epochs, warmup_epochs, decay_epochs,
                 steps_per_epoch: int = 2000):
        super().__init__(nominal_value=weight_decay, n_epochs=n_epochs,
                         warmup_epochs=warmup_epochs,
                         decay_epochs=decay_epochs,
                         steps_per_epoch=steps_per_epoch)
