...
class CharacterizationPipeline:
    ...
    def transform_csm(self, data):
        data["csm"] = (
            data["csm"]
            / data["csm"][:, self.ref_mic_index, self.ref_mic_index][
    ...
    def recover_csm(self, csm):
        csm = (
            csm
            * csm[:, self.ref_mic_index, self.ref_mic_index][:, tf.newaxis, tf.newaxis]
        )
        return csm
    ...
    def get_lambda_v(self, data):
        ...
        lambda_v = tf.reshape(
            lambda_v, [-1, input_shape[1], input_shape[2] * input_shape[3]]
        )
        data["csm"] = lambda_v
        return data
    ...
    def get_pipeline(
        self, dataset, batchsize, shuffle, shuffle_buffer_size, seed, **kwargs
    ):
        ...
        if self.freq_enc:
            ...
            if self.freq_enc_type == "ind":
                ...
                freq_enc = tf.constant(
                    [ap * fftc / c for fftc in freqs], dtype=tf.float32
                )
        ...
        if self.freq_enc and self.preprocess:
            tf_dataset = tf_dataset.padded_batch(
                ...
                padded_shapes=(
                    ((neig, num_mics * 2), ()),
                    ((max_nsources), (self.dim, max_nsources)),
                ),
                padding_values=((tf.constant(0, dtype=tf.float32), 0.0), (0.0, 0.0)),
                drop_remainder=True,
            )
            ...
            tf_dataset = tf_dataset.padded_batch(
                ...
                padding_values=(tf.constant(0, dtype=tf.float32), (0.0, 0.0)),
                drop_remainder=True,
            )
            ...
            tf_dataset = tf_dataset.padded_batch(
                ...
                padding_values=((tf.constant(0, dtype=tf.complex64), 0.0), (0.0, 0.0)),
                drop_remainder=True,
            )
            ...
            tf_dataset = tf_dataset.padded_batch(
                ...
                padding_values=(tf.constant(0, dtype=tf.complex64), (0.0, 0.0)),
                drop_remainder=True,
            )
        return tf_dataset
    ...
    def filter_kwargs(dataset, **kwargs):
        ...
        if kwargs.get("f") is None and "he_min" in kwargs and "he_max" in kwargs:
            ...
            raise ValueError(
                "f is not defined in kwargs, and he_min and/or he_max are not defined either"
            )
        else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(kwargs)
        dataset = self.get_pipeline(
            dataset, batchsize, shuffle, shuffle_buffer_size, seed, **kwargs
        )
        if cache:
