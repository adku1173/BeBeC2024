...
def main(config):
    ...
    validation_splits = [d.get_validation_pipeline() for d in config.datasets if d.validation]
    ...
    if config.ckpt:
        ckpt = tf.keras.callbacks.ModelCheckpoint(
            filepath=str(Path(config.ckpt_dir) / 'best_ckpt' / '{epoch:04d}-{val_loss:.2f}.keras'),
            monitor='val_loss',
